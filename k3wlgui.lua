...
-- Fling: rapidly moves your own HumanoidRootPart via direct CFrame changes
-- (kinematic, not physics forces), so it can never launch YOU — nothing ever
-- touches your own velocity or applies torque to your own body. Anyone whose
-- character overlaps you while this is happening still gets shoved hard,
-- because Roblox's physics engine pushes other unanchored parts out of the
-- way when they'd otherwise interpenetrate a moving part, repeated many
-- times a second. This is genuinely visible to others with no server script.
...
Commands["fling"] = function()
	...
	if flingMode == "walk" then
		-- Tiny, extremely fast back-and-forth nudges — too small and too fast
		-- to notice visually (looks like standing still), but the rapid
		-- repeated position changes still shove anything touching you hard.
		flingConn = RunService.Heartbeat:Connect(function()
			...
			h.CFrame = h.CFrame + offset
		end)
	else
		-- Visible fast spin in place.
		flingConn = RunService.Heartbeat:Connect(function(dt)
			local c = LocalPlayer.Character
			local h = c and c:FindFirstChild("HumanoidRootPart")
			if not h then return end
			h.CFrame = h.CFrame * CFrame.Angles(0, math.rad(2200) * dt, 0)
		end)
	end
end
