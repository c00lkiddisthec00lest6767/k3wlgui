...
--
-- Spin mode uses real BodyAngularVelocity so the physics engine actually
-- registers a velocity at your surface (kinematic CFrame spinning doesn't
-- count as real velocity for collision purposes, so it wouldn't push
-- anyone). A BodyPosition continuously re-anchored to your current spot
-- damps out the runaway self-launch that raw high torque used to cause,
-- without freezing your ability to walk.
--
-- Walk mode nudges your position by a tiny random amount every frame, but
-- always undoes the PREVIOUS frame's nudge first — so it oscillates around
-- your true position instead of compounding into a random walk that drifts
-- you away over time.
local flingMode = "spin" -- "spin" or "walk"
local flingActive = false
local flingConn
local flingLastOffset = Vector3.new(0, 0, 0)
...
local function stopFlingEffect()
	...
	local hrp = char and char:FindFirstChild("HumanoidRootPart")
	if hrp then
		if flingLastOffset.Magnitude > 0 then
			hrp.CFrame = hrp.CFrame - flingLastOffset
			flingLastOffset = Vector3.new(0, 0, 0)
		end
		local anchor = hrp:FindFirstChild("k3wl_FlingAnchor")
...
Commands["fling"] = function()
	...
	if flingMode == "walk" then
		...
		flingConn = RunService.Heartbeat:Connect(function()
			...
			-- Undo last frame's nudge first so it never accumulates.
			h.CFrame = h.CFrame - flingLastOffset
			local newOffset = Vector3.new(
				math.random(-100, 100),
				0,
				math.random(-100, 100)
			) / 100 * 0.6
			h.CFrame = h.CFrame + newOffset
			flingLastOffset = newOffset
		end)
	else
		local anchor = Instance.new("BodyPosition")
		anchor.Name = "k3wl_FlingAnchor"
		...
		spin.AngularVelocity = Vector3.new(0, 25, 0) -- fast, stable spin
		...
		flingConn = RunService.Heartbeat:Connect(function()
			local c = LocalPlayer.Character
			local h = c and c:FindFirstChild("HumanoidRootPart")
			local a = h and h:FindFirstChild("k3wl_FlingAnchor")
			if h and a then
