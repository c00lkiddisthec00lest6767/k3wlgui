...
-- Fling: spins your own HumanoidRootPart extremely fast, or (walk fling
-- mode) rapidly randomizes your own velocity. Either way, this is genuinely
-- visible to other players without any server script: the collision physics
-- get resolved on YOUR client the instant someone touches you, and their own
-- client picks up the resulting velocity and sends it onward normally — the
-- same way any two unanchored parts colliding works in Roblox.
...
local function stopFlingEffect()
	...
	local hrp = char and char:FindFirstChild("HumanoidRootPart")
	if hrp then
		local bav = hrp:FindFirstChild("k3wl_FlingSpin")
...
Commands["fling"] = function()
	...
	if flingMode == "walk" then
		-- No visible spinning or jitter at all — you look completely normal
		-- standing still. The instant something touches your HumanoidRootPart,
		-- it fires a single hard velocity burst and immediately resets, so the
		-- fling happens on contact without any telltale visible motion.
		flingConn = hrp.Touched:Connect(function(otherPart)
			...
			local dir = Vector3.new(math.random(-10, 10), 10, math.random(-10, 10))
			if dir.Magnitude == 0 then dir = Vector3.new(0, 1, 0) end
			myHRP.AssemblyLinearVelocity = dir.Unit * 400
