...
Commands["noclip"] = function()
	...
	noclipConn = RunService.Stepped:Connect(function()
		...
		-- Turn collision off on every part so you can walk straight through
		...
		-- Nothing's holding you up anymore once collision is off, so cancel
		-- out gravity's downward pull each frame — you float in place
		-- instead of falling through the floor into the void. Moving with
		-- WASD/jump still repositions you normally.
		if hrp then
...
local function scaleCharacter(factor)
	local char = LocalPlayer.Character
	if not char then return end
	-- Model:ScaleTo handles both R6 and R15 rigs correctly (adjusts every
	-- part and joint proportionally). The old approach only touched R15-only
	-- scale values, so it silently did nothing on R6 characters.
	pcall(function() char:ScaleTo(factor) end)
end
...
Commands["glow"] = function()
	...
	local hrp = char:FindFirstChild("HumanoidRootPart")
	if not hrp then return end
	local existing = hrp:FindFirstChild("k3wl_Glow")
...
-- Fling: two modes, both without a server script.
--
-- Both modes temporarily make your own HumanoidRootPart much denser
-- (heavier) while active. Momentum transferred in a collision scales with
-- mass — a heavier-but-slower you still sends a normal-weight player flying
-- hard, while moving much less yourself. This lets us use gentler velocity/
-- torque than before (avoiding the earlier self-launch chaos) while keeping
-- the actual fling force strong.
--
-- Walk mode fires a brief real velocity burst the instant something touches
-- you, aimed through the point of contact, then resets on the very next
-- physics frame (as fast as Roblox can possibly reset it) to minimize how
-- far you visibly move. There's still a small unavoidable jolt — making the
-- physics genuinely real for the other player means it's real for you too,
-- for that one instant.
local flingMode = "spin" -- "spin" or "walk"
local flingActive = false
local flingConn
local flingOriginalProps

local function stopFlingEffect()
	flingActive = false
	if flingConn then
	...
	local hrp = char and char:FindFirstChild("HumanoidRootPart")
	if hrp then
		local spin = hrp:FindFirstChild("k3wl_FlingSpin")
		if spin then spin:Destroy() end
		hrp.CustomPhysicalProperties = flingOriginalProps
		local v = hrp.AssemblyLinearVelocity
		hrp.AssemblyLinearVelocity = Vector3.new(0, v.Y, 0)
	end
	flingOriginalProps = nil
end
...
Commands["fling"] = function()
	...
	flingOriginalProps = hrp.CustomPhysicalProperties
	hrp.CustomPhysicalProperties = PhysicalProperties.new(35, 0.3, 0.4, 100, 100)

	if flingMode == "walk" then
		local onCooldown = false
		flingConn = hrp.Touched:Connect(function(otherPart)
			if not flingActive or onCooldown then return end
			local otherModel = otherPart:FindFirstAncestorOfClass("Model")
			local otherPlayer = otherModel and Players:GetPlayerFromCharacter(otherModel)
			...
			-- Cooldown prevents Touched re-firing repeatedly while the two
			-- characters are still overlapping, which was stacking multiple
			-- bursts on top of each other into something much more violent
			-- than intended.
			onCooldown = true
			task.delay(0.6, function() onCooldown = false end)

			local away = h.Position - otherPart.Position
			local flat = Vector3.new(away.X, 0, away.Z)
			local dir = flat.Magnitude > 0.01 and -flat.Unit or Vector3.new(0, 0, 1)
			h.AssemblyLinearVelocity = (dir * 140) + Vector3.new(0, 25, 0)

			-- Reset on the very next physics frame — as fast as possible —
			-- instead of a fixed real-time delay, to keep your own visible
			-- displacement to an absolute minimum.
			local resetConn
			resetConn = RunService.Heartbeat:Connect(function()
				resetConn:Disconnect()
				local c2 = LocalPlayer.Character
				local h2 = c2 and c2:FindFirstChild("HumanoidRootPart")
				if h2 then h2.AssemblyLinearVelocity = Vector3.new(0, 0, 0) end
			end)
		end)
	else
		-- Visual spin only — modest torque, just makes you visibly rotate.
		local spin = Instance.new("BodyAngularVelocity")
		spin.Name = "k3wl_FlingSpin"
		spin.AngularVelocity = Vector3.new(0, 15, 0)
		spin.MaxTorque = Vector3.new(0, 5e4, 0)
		spin.P = 5000
		spin.Parent = hrp

		-- The actual fling force: real linear velocity (the same mechanism
		-- that's already proven to work in walk mode), spun around in a
		-- circle. Because the direction constantly reverses, your net
		-- position stays roughly put even though the instantaneous velocity
		-- is always genuinely high — which is what touching physics reads.
		local spinAngle = 0
		flingConn = RunService.Heartbeat:Connect(function(dt)
			local c = LocalPlayer.Character
			local h = c and c:FindFirstChild("HumanoidRootPart")
			if not h then return end
			spinAngle = spinAngle + dt * 14
			local speed = 90
			h.AssemblyLinearVelocity = Vector3.new(
				math.cos(spinAngle) * speed,
				h.AssemblyLinearVelocity.Y,
				math.sin(spinAngle) * speed
			)
		end)
	end
end
...
local trollOptionsRow = Instance.new("Frame")
trollOptionsRow.Size = UDim2.new(1, -32, 0, 30)
trollOptionsRow.Position = UDim2.new(0, 16, 0, 44)
trollOptionsRow.BackgroundTransparency = 1
trollOptionsRow.Visible = false
trollOptionsRow.Parent = content

local trollOptionsLabel = Instance.new("TextLabel")
trollOptionsLabel.Size = UDim2.new(0, 80, 1, 0)
trollOptionsLabel.BackgroundTransparency = 1
trollOptionsLabel.Font = FONT
trollOptionsLabel.TextSize = 12
trollOptionsLabel.TextColor3 = COLOR_SUBTEXT
trollOptionsLabel.TextXAlignment = Enum.TextXAlignment.Left
trollOptionsLabel.Text = "Fling mode:"
trollOptionsLabel.Parent = trollOptionsRow

local flingModeButtons = {}

local function refreshFlingModeButtons()
	for _, entry in ipairs(flingModeButtons) do
		local isActive = entry.mode == flingMode
		entry.button.BackgroundColor3 = isActive and COLOR_ACCENT or COLOR_INPUT
		entry.label.TextColor3 = isActive and COLOR_TEXT or COLOR_SUBTEXT
	end
end

local function buildFlingModeButton(label, mode, xOffset)
	local btn = Instance.new("TextButton")
	btn.Size = UDim2.new(0, 100, 1, 0)
	btn.Position = UDim2.new(0, xOffset, 0, 0)
	btn.BackgroundColor3 = COLOR_INPUT
	btn.AutoButtonColor = false
	btn.Text = ""
	btn.Parent = trollOptionsRow
	...
	btn.MouseButton1Click:Connect(function()
		flingMode = mode
		refreshFlingModeButtons()
	end)

	table.insert(flingModeButtons, {mode = mode, button = btn, label = lbl})
end

buildFlingModeButton("Spin Fling", "spin", 86)
buildFlingModeButton("Walk Fling", "walk", 192)
refreshFlingModeButtons()
...
local function selectCategory(cat)
	...
	local isTroll = cat.name == "Troll"
	contentHeader.Visible = not isHome
	rowList.Visible = not isHome
	valueBox.Visible = not isHome and not isESP and not isTroll
	espColorRow.Visible = isESP and not isHome
	trollOptionsRow.Visible = isTroll and not isHome
...
do
	...
	local DRAG_THRESHOLD = isMobile and 22 or 8
