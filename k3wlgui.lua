...
--
-- Spin mode uses real BodyAngularVelocity — a real physics rotation the
-- engine tracks as genuine velocity, so touching it actually pushes people.
-- No position anchor this time: that was fighting your own walking every
-- frame. The earlier self-launch chaos came from torque set absurdly high
-- (500 rad/s, near-infinite torque) — much gentler values here stay stable
-- without needing an anchor to contain it.
--
-- Walk mode fires a brief real velocity burst the instant something touches
-- you, aimed through the point of contact, then resets almost immediately.
-- This is the only approach that reliably registers as real physics for
-- Roblox's collision engine — plain CFrame nudging (tried earlier) doesn't
-- count as real velocity, so it never actually pushed anyone despite looking
-- like screen shake. The trade-off: you'll feel a small jolt yourself in the
-- instant of contact, since making the physics genuinely real for the other
-- player means it's real for you too — there's no way to fake it as one-sided
-- without a server script.
...
Commands["fling"] = function()
	...
	if flingMode == "walk" then
		...
		flingConn = hrp.Touched:Connect(function(otherPart)
			...
			h.AssemblyLinearVelocity = (dir * 300) + Vector3.new(0, 40, 0)
		...
		spin.AngularVelocity = Vector3.new(0, 30, 0) -- fast, forceful spin
